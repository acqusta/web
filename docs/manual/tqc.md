...
建议安装目录
- Widnows 安装在d:\tquant
- Linux和Mac安装在 $HOME/opt/tquant目录
...
tqc主程序的配置文件是 etc/tqc.conf。通常不需要修改，只有当端口冲突再自行修改。
...
请用**Chrome浏览器**打开 [http://127.0.0.1:7080]，使用tqc的免费帐号即可以登录。如果没有帐号，请先在www.acqusta.com上免费注册一个帐号。tqc的使用是免费的，只有订阅了tquant的数据服务才会收些网络流量费用。
...
登录成功后，显示用户的服务订阅情况，如果没有使用云服务，可以忽略这些信息。
