...
为了提高性能，TQuantApi使用了C++扩展库。在Windows上编译需要安装Visual C++等编译工具，因此不建议Windows用户直接从源码安装。

**TQuantApi目前只支持Python2.7。如果必须要使用Python3，请耐心等待新版本。**
...
> IPC通信比TCP快，在行情推送上有优势，缺点是只能在本机使用。tqc缺省不开通TCP服务，
...
> API都返回两个值可以提醒序员要处理错误，养成好的编程习惯。

## 取行情前先订阅代码

第一次使用某个代码的行情前，记得先订阅，否则会因为tqc没有行情报错。代码格式采用"交易代码 + '.' + 市场代码"，如000001.SH, 000001.SZ, IF1801.CFE等，这样可以多市场统一编码。如果订阅成功，返回所有订阅的列表
...
下单的参数包括account_id, code, size, price和action。如果下单成功，返回委托号(entrust_no)或者自定义的订单编号(order_id)。对于股票交易，通常直接返回委托号。对于期货交易，由于下单的过程是异步的，会返回订单编号。详细说明请参考《期货交易中异步订单机制》。

下单接口中的size的单位是股，价格是元。
