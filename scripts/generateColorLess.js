...
const entry = path.join(antd, 'node_modules/antd/lib/style/index.less');
let content = fs.readFileSync(entry).toString();
const styles = glob.sync(path.join(antd, 'node_modules/antd/lib/*/style/index.less'));
content += '\n';
...
less.render.call(less, content, {
  paths: [path.join(antd, 'node_modules/antd/lib/style')],
}).then(({ css }) => {
  return postcss([
    reducePlugin,
  ...
  const bezierEasing = fs.readFileSync(path.join(antd, 'node_modules/antd/lib/style/color/bezierEasing.less')).toString();
  const tinyColor = fs.readFileSync(path.join(antd, 'node_modules/antd/lib/style/color/tinyColor.less')).toString();
  const colorPalette = fs.readFileSync(path.join(antd, 'node_modules/antd/lib/style/color/colorPalette.less'))
    .toString()
